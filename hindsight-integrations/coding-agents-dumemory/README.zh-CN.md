# dumemory-coding-agents（中文说明文档）

为 **Coding Agents（编码智能体）** 打造的长期项目记忆引擎，由 [DuMemory](https://cloud.memory.bj.baidubce.com) 提供底层技术支持。  
一套统一包，适配各大主流 Agent：共享底层的推理与注入（Reflect-and-Inject）核心，并为每个 Agent 提供轻量级接入入口（**opencode**、**opencode 2**、**Kilo CLI**、**Cline CLI**、**pi**、**Prime Agent**、**DeepSeek Harness**、**Claude Code**、**Codex CLI**、**DeepAgents Dcode**、**Antigravity CLI**、**Cursor CLI**、**GitHub Copilot CLI**、**Devin CLI**、**Grok Build**、**Qwen Code**）。数据接入完全全自动 —— 无需繁琐的设置命令：在你日常编码工作的过程中，仓库的 Git 历史记录与对话会话将在后台静默流式汇入专属的 Memory Bank（记忆库）。
...
```bash
npx @baiducloud/dumemory-coding-agents install all          # 为所有检测到的 Agent 原生配置记忆插件
npx @baiducloud/dumemory-coding-agents install claude-code  # 或仅安装指定的一个 Agent
npx @baiducloud/dumemory-coding-agents uninstall all        # 严格清理安装时新增的内容
npx @baiducloud/dumemory-coding-agents update               # 仅刷新运行时暂存，不改写宿主配置
```

`install` 命令需要显式指定目标 —— `all`，或者一个/多个具体的 Harness 名称。直接执行裸命令 `npx @baiducloud/dumemory-coding-agents install` 不会改动任何配置，仅会打印可选列表，避免意外为整台机器上的所有 Agent 盲目配置。**更新也是运行同样的 `install` 命令** —— 它会在原地重新复制最新的运行时代码。
...
在终端交互执行时，安装器会询问你的 **API Token**；脚本化无人值守安装可通过 `--api-token <token>` 传入（参见 [记忆存储位置](#记忆存储位置-where-memory-lives)）。该问题只在首次安装时询问一次，后续重复安装不会重复打扰。
...
安装器会将本包注册为 Dcode 的本地插件市场源，随后调用 Dcode 原生的 `plugin install` 命令。本插件遵循原生 Agent Plugin 规范：其根目录的 `plugin.json` 提供了共享技能、Hooks V2 的 `SessionStart`、`UserPromptSubmit` 与 `Stop` 生命周期，以及命名空间化的 `dumemory_*` MCP 服务。安装后通过 Dcode 标准的插件管理器启用即可，无需任何 Dcode 配置补丁或兼容层。

Dcode 会为其插件的 MCP 工具加上命名空间前缀，因此工具名会显示为 `plugin__dumemory-coding-…__dumemory_…` 而非原始短名 —— Agent 无论如何都能根据工具向导正确解析调用。在无头自动化运行（`dcode -n`）时，Dcode 允许只读类 DuMemory 工具运行，但会将两个写入类工具（`dumemory_ingest_document`、`dumemory_capture_initiative`）拦截在审批门禁之后（无头模式下因缺少 UI 无法审批）；因此，捕获倡议或存入文档请使用交互式 TUI 界面。同理，首次单次代码库调研（Codebase Survey）若检测到系统中安装了其他 Agent CLI，会自动借用其无头进程执行（与 Cursor、Copilot、Devin、Grok Build、Cline、Kilo 和 Prime Agent 的借用策略完全一致）。
...
与 v1 相比有两处差异（皆由宿主特性决定）：首次单次代码库调研会借用其他已安装 Agent 的 CLI 执行（v2 插件目前无法定义调研所需的只读子 Agent）；种子灌库 Banner 写入插件日志中，而非 TUI 吐司弹窗（v2 插件无法抛出弹窗）。除此之外，Recall 召回、提示词注入、原生 `dumemory_*` 工具以及会话回写功能与 v1 完全相同。
...
向 `~/.config/kilo/kilo.json` 中写入插件条目。
...
配置 `~/.copilot/hooks/`、`mcp-servers/dumemory-coding-agents.json`，并安装伴生技能。
...
配置生命周期 Hook、MCP 服务，以及底部的 `DuMemory · <bank>` 终端状态行（Statusline）。
...
在 `~/.devin/hooks.json` 与 `~/.devin/mcp.json` 中配置 Hook 及 MCP。**要求 Node 22.5 或更高版本** —— 详见下文说明。
...
此命令是唯一官方支持的安装路径（对下文的 Prime Agent 同理）。我们故意没有将本插件作为标准 pi 包进行分发（`pi install npm:@baiducloud/dumemory-coding-agents`）：因为 pi 与 Prime Agent 会读取 `package.json` 中的同一个 `pi` 键，而该键只能指定一个主入口 —— 无论未命中的宿主是哪一个，都会错误加载另一个宿主的 Bundle 并将自己汇报为错误的 Agent，导致它误读别的 Harness 配置分区并错误盖戳。因此本包完全没有包含 `pi` 键，而是由上面的安装命令将每个宿主精准指向其专属的 Bundle。
...
`install` 会将运行所需文件复制到 `~/.dumemory/coding-agents` 并将各 Agent 的配置指向该路径，因此运行时不依赖于最初执行安装命令时的所在目录。**更新只需再次运行相同的安装命令** —— 它会在原地重新复制运行时代码，保持已有配置有效，并让所有新会话无缝运行在最新版本上。

`install` 会以幂等方式安全合并配置到各 Agent 的原生配置文件中，完整保留用户原有的所有字段；在修改前会自动将目标文件备份为 `<file>.dumemory-backup`。`uninstall` 仅精准清理本插件添加的条目。针对 Claude Code，安装还会部署一份**伴生技能（Companion Skill）**（`~/.claude/skills/dumemory-coding-agent`），指导 Agent 理解记忆系统的工作方式 —— 例如“将这部分存入 dumemory”该如何执行、可用的工具体系、单仓库配置定制、排错诊断等，开发者可以直接在对话中向 Agent 询问。
...
- **Claude Code** 与 **Codex**：使用自带的安装命令配置完整的三个 Hook + MCP 服务。本包暴露的 `bin` 命令（`dumemory-claude-hook`、`dumemory-codex-hook`、`dumemory-cursor-hook`）是供极简手工配置使用的单个提示词注入入口（`UserPromptSubmit`）。
...
## 导入已有历史会话（Importing existing conversations）

全新安装后，记忆只来自 Git 历史，并不知道你此前与 Agent 讨论过什么。`--import-conversations` 会把 Agent 已经写在本机磁盘上的会话转录重新提取为新文档：

```bash
cd /path/to/your/repo
npx @baiducloud/dumemory-coding-agents install claude-code --import-conversations   # 或: install codex --import-conversations
```

消耗的 Token 与导入的历史规模大致成正比。该操作可安全重复执行（数据接入层依据文档 ID 进行严格去重）。
...
**一个仓库一个 Bank**：记忆按代码仓库归集（`coding-agent::{gitProject}`），供所有 Agent 共享，因此中途切换 Agent 仍然读到同一份记忆。如需按 Agent 隔离，可在配置中声明：

```jsonc
{ "bankIdTemplate": "{harness}::{gitProject}" } // 每个 Agent 每个仓库一个独立 Bank
```
...
记忆统一存储在 **Baidu AI Cloud**。终端安装时会交互询问 API Token；自动化脚本直接传参即可：
...
如需接入**你自己私有化部署**的同款 API，用 `--api-url` 指向它即可 —— URL 下游的所有逻辑完全一致，私有化部署无需任何额外配置：
...
| :----------- | :--------------------- | :----------------------------------------- | :------------------------ |
| `serverMode` | `DUMEMORY_SERVER_MODE` | `cloud`                                    | `cloud` —— 唯一支持的模式 |
| `apiUrl`     | `DUMEMORY_API_URL`     | `https://cloud.memory.bj.baidubce.com/api` | 实际访问的 API 基础地址   |
| `apiToken`   | `DUMEMORY_API_TOKEN`   | —                                          | Bearer 鉴权 Token，必填   |
...
所有配置收敛在**单一 JSON 文件**中：`~/.dumemory/coding-agent.json`。生效优先级由低到高如下（后者覆盖前者）：

1. 内置默认值（Built-in Defaults）
2. 环境变量 —— `DUMEMORY_API_URL`、`DUMEMORY_API_TOKEN`，以及各个标量设置（大写蛇形命名 `DUMEMORY_<FIELD_IN_CAPS>`），便于容器与 CI 流水线注入配置；
...
环境变量扮演的是**保底回退（Fallback）**角色：只要配置文件中显式配置了某项，文件永远优先；因此在已有配置文件的机器上声明环境变量不会产生意外篡改。两个列表类型的参数（`retainTags` 和 `optInPaths`）支持逗号分隔传递（如 `DUMEMORY_RETAIN_TAGS="project:{gitProject},env:work"`），系统会自动执行去除空白。  
字典映射类的配置（`mapPathToBank`、`harnesses`、`banks`、`retainMetadata`）由于无法扁平展开为单一变量，仅支持在 JSON 文件中定义。`maxParallelRetains` 支持通过 `DUMEMORY_MAX_PARALLEL_RETAINS` 供容器和 CI 调节并发。

环境变量 `DUMEMORY_CONFIG` 可用于自定义配置文件的存放路径 —— 在容器或测试沙箱中，如果 `$HOME` 并非合适的锚点，可将其重定向到自定义路径（依然是单个文件，仅改变其磁盘位置）。其他不属于业务设置的环境变量包括 `DUMEMORY_LOG_FILE`、`DUMEMORY_DIAG_FILE` 与 `DUMEMORY_LOG_LEVEL`（参见 [诊断与日志](#诊断与日志-diagnostics--logging)）。
...
| **工具层背后的 MCP Server**<br>（为 `dumemory_*` 工具提供服务的子进程）                                                   | 进程启动时加载一次                                             | **下一次打开新会话窗口**时生效 |
...
调用 `dumemory_diagnose` 工具可以同时打印两端的快照 —— 既能看到当前磁盘配置文件的最新内容，也能看到内存中活跃客户端当前实际生效的值。
...
| :---------------------- | :----------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `apiUrl`                | `https://cloud.memory.bj.baidubce.com/api` | DuMemory Cloud API 基础地址。                                                                                                                                                                                                                                                                                                   |
...
| `reflectTimeoutMs`      | `28000` (ms)                               | **自动会话 Reflect 超时**，所有 Agent 一律按此值执行，不再有任何按 Agent 的封顶。默认值取得能装进本插件写入宿主的 30 秒 Prompt Hook 超时内（并为 Hook 的其余工作留余量）；因此在 Hook 类 Agent 上调高它，可能导致宿主在 Reflect 途中杀掉 Hook，而常驻插件型 Agent 没有这个窗口限制。超时后会话优雅降级在无 Reflect 状态下运行。 |
| `reflectToolTimeoutMs`  | `330000` (ms)                              | 大模型主动调用 `dumemory_reflect` 时的超时上限。默认高于服务端的保护超时（300 秒），将放弃决策权交由服务端。                                                                                                                                                                                                                    |
| `reflectBudget`         | `"high"`                                   | 模型调用 `dumemory_reflect` 时的推理预算：`"low"`、`"mid"` 或 `"high"`。大规模知识库若遭遇超时可适度调低。自动 SessionStart 始终使用 `"low"`。                                                                                                                                                                                  |
| `autoReflect`           | `true`                                     | 会话**首轮提问**时是否自动推理注入 `<dumemory_memory>`。若设为 `false`，则关闭隐式注入，转为纯工具主动按需调用。                                                                                                                                                                                                                |
...
| `autoUpdate`            | `true`                                     | 每天在会话启动时后台静默检查一次（通过 `npm view`，因此遵循用户配置的镜像源或私有源），发现新版本时热更 `~/.dumemory/coding-agents`。设为 `false` 可锁定版本。                                                                                                                                                                  |
...
`pageTriggerType`/`pageTriggerCron` 仅决定知识页在**何时（When）**刷新；至于**如何（How）**刷新则由服务端控制：DuMemory 对知识页采用增量更新算法（每次迭代是在旧页面上修正，而非全量重推），上述参数是与服务端的默认参数进行合并。
...
3. **动态计算** —— 解析 `bankIdTemplate` 中的占位符：
   ...
   - `{channel}` / `{user}` —— 来自环境变量 `$DUMEMORY_CHANNEL_ID` / `$DUMEMORY_USER_ID`。
...
然而，它们并不是合理的观察隔离边界。服务端默认的 `combined` 模式会为每个不同的标签组合构建独立的一套观察集 —— 这会导致同一代码库如果在两个 Agent 之间交替编码，就会分化出两套永不合并、彼此盲区且计算成本翻倍的平行信念集。无论当时是哪个 Agent 在敲键盘，项目的代码规范与架构决策都应当是唯一的客观事实。
...
数据摄取没有面向用户的独立命令 —— 会话启动时在后台静默拉起深化引擎（`dist/deepen.js`），自动按需补齐缺失的工作：Bank 底层策略配置、历史会话导入（基于文档 ID 去重）、单次 Gitlog 种子灌库、增量分批 Commit Diff 深度挖掘（由新至旧，单次限制批次规模），并在提取完成后驱动知识页面合成。需要严格确定性测试的场景（基准评测、E2E 测试套件）可直接运行深化引擎，并轮询 `dist/status.js` 直到 `"synced": true` —— 这与 Agent 工具 `dumemory_sync_status` 的就绪判定契约完全一致。
...
`src/docs-freshness.test.ts` 是一道防漂移自动化门禁：当技能陈旧未重新构建、或代码中新增了配置字段却未在 README 中体现时，测试会直接失败阻断提交。
...
1. **分级插件运行日志（供人工调试）**：`$TMPDIR/dumemory-coding-agent/plugin.log`（可通过 `DUMEMORY_LOG_FILE` 覆盖） —— 记录带时间戳与模块标签的日志输出。默认级别为 `info`；可在配置中设置 `"logLevel": "debug"` 或在环境变量声明 `DUMEMORY_LOG_LEVEL=debug` 查看极详尽的执行流。
2. **结构化诊断事件流（供程序/自动化 Harness 消费）**：每次 Reflect 推理与知识页面读取的结果均以 JSONL 形式单行追加到 `/tmp/dumemory-plugin.log`（可通过 `DUMEMORY_DIAG_FILE` 覆盖）：
   ```json
   {
...
调用面向 Agent 的工具 `dumemory_sync_status`（脚本调用 `dist/status.js`）：返回 `"synced": true` 表示冷启动种子记忆已经完全就绪并可供检索。它同时会返回 Gitlog 新鲜度、Commit Diff 深化推进到的具体位置、代码库调研状态（`surveyBaseline` 代表开始调研时的 Commit HEAD，`surveyDocs` 显示已入库的发现文档数量 0~4 篇 —— 缺少文档会自动重试），以及后台仍在跑的任务数。
