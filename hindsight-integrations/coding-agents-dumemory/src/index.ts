/**
 * dumemory-coding-agents — long-term memory for coding agents (recall + INJECT), harness-pluggable.
 *
 ...
 *   READ   — each user turn, recall on the prompt and PUSH a `<dumemory_memory>` block (with the
 ...
 *   TOOLS  — register the dumemory_* knowledge/recall suite natively (no MCP server needed).
 ...
 * binds it to opencode's plugin API. All configuration comes from ~/.dumemory/coding-agent.json
...
const DuMemoryCodingAgentsPlugin = createPluginEntry("opencode");

export default DuMemoryCodingAgentsPlugin;
export { DuMemoryCodingAgentsPlugin };
