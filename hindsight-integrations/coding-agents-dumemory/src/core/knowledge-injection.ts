...
/** Defensive parse of DuMemoryClient.listPages() ({items:[{id,name}]}, flattened from the
 *  knowledge-base tree). The ids are knowledge-base node ids — the same id space the agent passes
 *  back to dumemory_read_knowledge_page. */
...
/**
 * When-to-call guide for the FULL DuMemory tool suite. Shared by the SessionStart preamble and the
 ...
 * context. (Omits dumemory_diagnose — pure troubleshooting, no workflow trigger.)
 */
const TOOL_GUIDE =
  "- dumemory_search_knowledge_pages(query) — FIRST STOP for any question the project's accumulated " +
  ...
  '"> 🧠 **From DuMemory (<page>)** — <the specific facts you drew on>".\n' +
  "- dumemory_list_knowledge_pages / dumemory_read_knowledge_page — BEFORE substantial work, list the pages and " +
  ...
  "- dumemory_reflect(query) — when pages are too shallow and you need the WHY: deep reasoning over the " +
  "repo's full memory for the past decision and exact values that explain a behavior or bug (slower — " +
  'use deliberately, and credit results with a blockquote header "> 🧠 **From DuMemory** — <summary>").\n' +
  "- dumemory_capture_initiative(title, summary) — right after the user approves a plan or finishes brainstorming a " +
  ...
  "- dumemory_ingest_document(title, content) — save an external document or durable notes/findings you want " +
...
const PAGES_FIRST_ON_GOALS =
  ...
  "dumemory_search_knowledge_pages. No synthesis is injected automatically in this configuration; " +
  "call dumemory_reflect only when those pages are too shallow and deeper reasoning is needed.\n";

export interface ToolGuideOpts {
  /** Add the new-goal pull trigger (tool-only reflect mode, cfg.autoReflect=false). It used to send
   *  the agent straight to dumemory_reflect; it now goes to the knowledge pages first and keeps
...
export function buildKnowledgePreamble(pages: PageRef[], opts?: ToolGuideOpts): string {
  const body = pages.length
    ...
    : "No knowledge pages yet — DuMemory is still learning this repo; they'll appear as it processes.";
  return (
    "<dumemory_knowledge>\n" +
    "This repository has a DuMemory memory + knowledge base (curated, continuously-updated pages plus the raw " +
    "memory behind them). The tools below are registered, but you must actually CALL them at the right moments:\n" +
    `${toolGuide(opts)}\n` +
    "ALSO your correction tool: when you verify a DuMemory memory is wrong or stale, ingest a " +
...
export function buildRosterRefresh(pages: PageRef[], opts?: ToolGuideOpts): string {
  const rosterBlock = pages.length
    ? `Current DuMemory knowledge pages (may have changed):\n${roster(pages)}\n`
    : "";
  return (
    "<dumemory_knowledge_refresh>\n" +
    rosterBlock +
    "Reminder — this repo's DuMemory tools are available; call them at the right moments:\n" +
    `${toolGuide(opts)}\n` +
    "</dumemory_knowledge_refresh>"
  );
}
