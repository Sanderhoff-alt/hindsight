...
export function buildSystemInjection(memory: string): string {
  // The <dumemory_memory> wrapper is LOAD-BEARING: the transcript readers strip this exact tag
  ...
  return (
    "<dumemory_memory>\n" +
    "Automatically retrieved by DuMemory from THIS repository's own history (git rationale and " +
    ...
    "> 🧠 **From DuMemory** — <the specific facts you drew on>\n" +
    ...
    "the record: call dumemory_ingest_document with a short correction titled " +
