# DuMemory Coding Agents 核心机制、知识库与多 Agent 集成深度剖析

本文档全面深入剖析了 DuMemory 统一多 Agent 插件（源码位于 `hindsight-integrations/coding-agents-dumemory`，npm 包名为 `@baiducloud/dumemory-coding-agents`）的核心架构设计、交互层四大支柱、8 大核心工具、知识库 OKF 规范与单树拓扑、系统基座层五大能力以及全量运行配置。
...
```
├── 1. 插件源码核心架构（hindsight-integrations/coding-agents-dumemory/）
...
```
hindsight-integrations/coding-agents-dumemory/
├── src/                          # 核心源码
│   ├── core/                     # 记忆系统核心逻辑实现
...
负责处理记忆存取、配置管理以及各大 Agent 转录文本的解析。

| 文件 / 模块              | 职责与功能说明                                                                                                                                                                                                                                                                      |
| :----------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `config.ts`              | 多层级配置加载解析器。管理`~/.dumemory/coding-agent.json`、项目根目录配置及环境变量。                                                                                                                                                                                               |
| `bank.ts`                | 动态 Bank 映射与管理。根据 Git 仓库根目录自动隔离或分配记忆库（Memory Bank）。                                                                                                                                                                                                      |
| `dumemory.ts`            | DuMemory 后端 API 交互客户端，负责知识页面检索、会话保留、Recall/Retain 协议实现。                                                                                                                                                                                                  |
| `knowledge-injection.ts` | 记忆与知识注入逻辑。在用户提问或会话启动时组装上下文提示词。                                                                                                                                                                                                                        |
| `knowledge-tools.ts`     | 暴露给各 Agent 的 MCP / 原生工具定义（`dumemory_recall`, `dumemory_retain` 等）。                                                                                                                                                                                                   |
...
>   1. **DuMemory 服务端 HTTP API**：由配置中的 `apiUrl` 指定（默认为 Baidu AI Cloud，也可指向企业私有部署服务端）；
...
- **统一安装/卸载/更新器**：`installer.ts` / `install-ui.ts` (`dumemory-coding-agents`)
- **通用 MCP 服务器**：`mcp-server.ts` (`@baiducloud/dumemory-coding-agents/mcp-server`)
...
- **诊断与状态监控**：`status.ts`
- **知识深化与初始化工具**：`deepen.ts`、`dumemory-seed.ts`
...
> 无论采用哪种机制，DuMemory 均为其同时提供 **“强制隐式拦截（生命周期 Hooks）”** 与 **“显式按需查询（工具 Tools）”** 双重机制。两者的根本差异在于宿主平台的系统开放度与技术承载形式：
...
>
> - **Hook 二进制只是瞬态短进程**：用户每次敲回车提问时启动数百毫秒，输出记忆文本后立即 `process.exit(0)` 退出销毁。当大模型在后续思考过程中想要主动调用 `dumemory_recall` 时，Hook 进程早已退出，且外部 Hook 规范本身无法向大模型注册工具函数；
...
- **`skill/`** 与 **`src/core/skill-dirs.ts`**：
  - 存放编译生成的伴生技能文件 `SKILL.md`，指导各 Agent 正确调用 DuMemory 记忆工具。
  - **分发策略与隔离设计**：
    ...
    - **共享标准路径**：Codex 与 DeepSeek Harness (DSH) 遵循开放规范，统一安装并共享 `~/.agents/skills/dumemory-coding-agent/`。
...
DuMemory 在交互层为各类 Coding Agent 建立了**“隐式自动托底 + 显式自主调用”**的完整交互闭环，由以下四大支柱协同构成：
...
- **注入时序机制（两类宿主差异）**：
  ...
  1. **支持 `SessionStart` 物理事件的 Agent（Claude Code, Cursor, Codex, Grok, Qwen 等）**：
     - **启动时（未敲字）**：触发 `SessionStart` 钩子，立即向初始会话上下文注入 `<dumemory_knowledge>`；
     - **第 1 轮提问（敲下回车）**：触发 `UserPromptSubmit` 钩子，拿着用户问题执行 Reflect 推理，将匹配出的 `<dumemory_memory>` 注入当前轮上下文。
  2. **无 `SessionStart` 事件的 Agent（OpenCode, Antigravity, Cline, Kilo 等）**：
     - 在第 1 轮发问时，系统将原本属于启动前言的 `<dumemory_knowledge>` 与针对该问题的 `<dumemory_memory>` **合并在第 1 轮对话中一次性注入**（见 `src/core/runtime.ts:L173`）。
  3. **后续轮次（第 2 轮及以后）**：决策事实不再重复注入；每隔固定轮次（`pageRefreshEveryTurns`）周期性追加 `<dumemory_knowledge_refresh>`。
- **仅有的三大强制注入切片时刻**：
  整个会话期间，系统严格收敛为**仅在以下 3 个时刻**触发强制注入，平时的普通对话轮次（如第 2、3、4 轮用户回复“好的”、“继续”、“修这里”等）**保持完全静默、零强制注入**：
  1. **会话启动（Session Start）**：注入 `<dumemory_knowledge>`，全会话仅 1 次（建立心智）；
  2. **首轮提问（First Prompt）**：注入 `<dumemory_memory>`，全会话仅 1 次（基于首句问题定下历史决策红线）；
  3. **长会话周期刷新（Every N Turns）**：注入 `<dumemory_knowledge_refresh>`，按固定周期触发（防注意力漂移与遗忘）。

| 注入时刻切片                                  | 注入内容块                                                 | 出现频次                                     | 核心工程目的                                                                          |
| :-------------------------------------------- | :--------------------------------------------------------- | :------------------------------------------- | :------------------------------------------------------------------------------------ |
| **1. Session Start**(或无该钩子时的第 1 轮)   | **`<dumemory_knowledge>`**(工具指南 + 架构页面清单)        | **整场会话仅 1 次**                          | 启动瞬间建立心智：“手头有哪些工具，项目已有哪些架构页面”                              |
| **2. 首次用户 Prompt**(第 1 轮提问敲下回车时) | **`<dumemory_memory>`**(当前任务相关的历史决策事实)        | **整场会话仅 1 次**(冷库自举可顺延至第 2 轮) | 拿用户首句真实意图做一次深度 Reflect 推理，把规约推到眼前定调。后续轮次绝不再重复注入 |
| **3. 周期性防疲劳刷新**(长会话每隔固定 N 轮)  | **`<dumemory_knowledge_refresh>`**(页面清单与工具重新唤醒) | **固定周期触发**(如每 5 轮或 10 轮一次)      | 刷新长会话中新增的页面，防止模型注意力漂移或在上下文压缩（Compaction）后遗忘工具      |
...
- **三大核心内容块（规范化 XML 结构）**：
  1. **`<dumemory_knowledge>`（知识底座与工具向导，见 `src/core/knowledge-injection.ts`）**：
  ...
  2. **`<dumemory_memory>`（项目历史决策与校准安全框架，见 `src/core/inject.ts`）**：
     - **防幻觉与相关性声明**：告知模型检索属于启发式，若不相关请直接彻底忽略；
     - **时态定性（Anti-Prompt Injection）**：明确这是“过去的记录而非现在的指令”，严禁将祈使句当成当前任务执行；
     - **强字面量采纳与显式归因**：凡涉及具体数字、映射表、规则，直接严格采纳，回答中必须输出 `> 🧠 **From DuMemory memory** — ...`；
     - **动态历史决策正文**：Reflect 返回的经陈述句严格约束的 Commit/PR 真实决策与字面量。
  3. **`<dumemory_knowledge_refresh>`（长会话防疲劳刷新）**：
     - 周期性追加最新知识页面清单，重新唤醒模型对 DuMemory 工具的主动调用意识。
...
- **收敛暴露的 8 大工具**（参见 `src/core/knowledge-tools.ts`）：
  1. `dumemory_search_knowledge_pages`：混合全文与语义搜索项目知识库（快速检索首选）；
  2. `dumemory_list_knowledge_pages` / `dumemory_read_knowledge_page`：结构化知识页面清单与内容读取（支持 `[[page:<id>]]` 递归引用跳转）；
  3. `dumemory_reflect`：深层多跳 Agentic 推理，探究“为什么这样写”的根因；
  4. `dumemory_capture_initiative`：方案确定后、写代码前，捕捉并持续跟踪一项新功能计划；
  5. `dumemory_ingest_document`：保存外部资料/设计稿，或作为纠错通道更新过时记忆；
  6. `dumemory_sync_status` / `dumemory_diagnose`：同步状态与运行时安全诊断。
...
- **核心规约**（参见 `skill/SKILL.md`）：
  ...
  2. **显式归因规范**：模型采纳记忆回答时，必须输出 `> 🧠 **From DuMemory memory (<page>)** — <所依据的事实>`；
  3. **自我愈合与纠错协议（Correction Protocol）**：若模型在当前代码事实中发现旧记忆错误或过时，严禁视而不见，必须调用 `dumemory_ingest_document` 写入 `Correction: <topic>` 进行覆盖治愈。
...
> - **“新文档 ➔ 异步刷页面”的双层自愈闭环**：
>   1. **第一层：即时检索压制（毫秒级生效）**：Agent 发现矛盾后调用 `dumemory_ingest_document` 提交包含三要素（旧说辞 + 新事实 + 代码证据）的修正文档。因时间戳最新且附带强证据，下次 `/reflect` 检索时后端算法会直接在推理和排序上**自动压制并覆盖旧记忆**；
...
- **Fail-Open 容灾原则**：所有 Hook 均具备超时保护与静默 try-catch，DuMemory 故障时绝不阻塞正常编码。
...
sequenceDiagram
    ...
    participant Server as DuMemory 记忆后端
    ...
    Hook->>Prompt: 生成 <dumemory_memory> 块
...
在 DuMemory 的底层设计中，所有工具均由核心工厂函数 `buildKnowledgeTools()` 统一收敛定义，并通过客户端 `DuMemoryClient` 与服务端的 REST API 进行 HTTP 通信。
...
在 DuMemory 的数据模型中，“**知识页（Knowledge Pages）**”与“**底层原始事实 / 记忆图谱（Raw Documents & Graph）**”具有明确的分工：
...
| :------------------------------------ | :----------- | :--------------------- | :---------------------------------------------------------- | :------------------------------------------ | :------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------- | :----------------------------------------- | :------------------------------------------------------------------------------------------------------------- |
| **`dumemory_search_knowledge_pages`** | **知识页**   | 知识库页面正文与摘要   | `query: string`                                             | `readOnlyHint: truedestructiveHint: false`  | `GET /knowledge-base/search?q={query}&limit=3`                                   | **混合检索首选**。服务端执行 BM25 + Vector（RRF 倒数排名融合），返回相关知识页及高亮代码片段。                       |                                            |                                                                                                                |
| **`dumemory_list_knowledge_pages`**   | **知识页**   | 知识库目录树           | _无参数_                                                    | `readOnlyHint: truedestructiveHint: false`  | `GET /knowledge-base/tree`                                                       | **枚举页面清单**。获取知识树（Folders + Pages），客户端自动过滤掉文件夹，返回所有知识页 ID、标题与更新时间。         |                                            |                                                                                                                |
| **`dumemory_read_knowledge_page`**    | **知识页**   | 指定知识页内容         | `page_id: string`                                           | `readOnlyHint: truedestructiveHint: false`  | `GET /knowledge-base/pages/{page_id}`                                            | **读取页面正文**。读取指定知识页的完整 OKF（Open Knowledge Format）格式，包含 YAML Frontmatter 及 Markdown 正文。    |                                            |                                                                                                                |
| **`dumemory_capture_initiative`**     | **知识页**   | 特性计划知识页         | `title: stringsummary: string``relates_to_page_id?: string` | `readOnlyHint: falsedestructiveHint: false` | ①`POST /knowledge-base/folders`② `POST /knowledge-base/pages`③ `POST /memories`  | **创建特性知识页**。方案确定后、写代码前调用。在 Initiatives 分类下新建知识页，并向记忆流写入一条关联 Marker 事实。  |                                            |                                                                                                                |
| **`dumemory_reflect`**                | **非知识页** | 全库因果实体与记忆图谱 | `query: stringbudget?: "low"\                               | "mid"\                                      | "high"`                                                                          | `readOnlyHint: truedestructiveHint: false`                                                                           | `POST /reflect`_(Body: `{query, budget}`)_ | **深度因果推理**。穿透静态页面，直接在全库历史 Commit、PR 和会话实体图谱中多跳搜索，解答“为什么当初这么设计”。 |
| **`dumemory_ingest_document`**        | **非知识页** | 原始文档与自愈纠错库   | `title: stringcontent: string`                              | `readOnlyHint: falsedestructiveHint: false` | `POST /memories`_(Body: `{items: [...], async: true}`)_                          | **事实存入与记忆纠错**。将技术笔记或 `Correction: <topic>` 纠错文档存入原始文档库（`source:upload`），触发异步提取。 |                                            |                                                                                                                |
| **`dumemory_sync_status`**            | **非知识页** | 后台任务与数据统计     | _无参数_                                                    | `readOnlyHint: truedestructiveHint: false`  | ①`GET /documents?tags=source:...`② `GET /knowledge-base/tree`③ `GET /operations` | **同步状态感知**。查询当前 Bank 中 Git、Chat、Upload 各类文档的导入量，并统计仍在跑的异步提取任务数。                |                                            |                                                                                                                |
| **`dumemory_diagnose`**               | **非知识页** | 本地环境与运行状态     | _无参数_                                                    | `readOnlyHint: truedestructiveHint: false`  | **无远程网络请求\***(纯本地诊断，必要时仅测 `/health`)\*                         | **本地诊断探针**。读取本地配置文件、环境变量覆盖、内存中的 Token 配置状态，不向外网回传任何 Secret 或数据。          |                                            |                                                                                                                |
...
flowchart TD
    ...
    Q2 -->|是 (已有 page_id)| T1["dumemory_read_knowledge_page<br>(读取单页完整 OKF 正文)"]
    Q2 -->|否 (需要语义或关键词探索)| T2["dumemory_search_knowledge_pages<br>(混合检索 RRF 关键词排名)"]
    Q2 -->|想浏览全库有哪些架构页| T3["dumemory_list_knowledge_pages<br>(枚举所有页面 ID 与标题)"]

    Q1 -->|否| Q3{"是否想探究代码为何这么写、<br>或寻找 Bug 的深层历史因果？"}
    Q3 -->|是 (深层溯源)| T4["dumemory_reflect<br>(穿透静态页面，多跳推理图谱)"]

    Q3 -->|否| Q4{"开发方案已讨论对齐，<br>准备开始编写大特性代码？"}
    Q4 -->|是 (锁定规划)| T5["dumemory_capture_initiative<br>(在 Initiatives/ 目录下创建追踪页)"]

    Q4 -->|否| Q5{"发现旧记忆与当前代码事实冲突，<br>需要进行自我治愈纠错？"}
    Q5 -->|是 (更正信念)| T6["dumemory_ingest_document<br>(以 'Correction: topic' 追加新证据)"]

    Q5 -->|否 (状态与排障)| T7["dumemory_sync_status / diagnose<br>(查看异步同步进度或本地排障)"]
```

## 4. 知识库系统全景设计（OKF 规范、单树拓扑与知识页分类）

在 DuMemory 的体系中，知识库（Knowledge Base）是代码库长期记忆的**高层凝练与静态结晶**。它将底层数千条散乱的 Git 提交 Diff、历史对话和临时笔记，经由大模型因果整合后，投影为一套结构化、可被开发者与所有 Agent 直观阅读的知识网络。
...
- **`Initiatives/`（物理文件夹）**：微观落地视图，由 `dumemory_capture_initiative` 工具按需动态创建（如 `initiative-gateway-v2.md`），记录该特性的完整方案设计、修改边界与当前状态；
- **自动化反向链接纽带**：
...
- **Frontmatter 6 大标准字段**：
  ...
  - `timestamp`：最后一次刷新时间戳。
- **服务端原生生成事实**：
  - YAML Frontmatter 是 DuMemory 服务端（`dumemory_api/api/page_markdown.py`）通过纯函数原生拼接渲染的，客户端插件仅做原样透传；
...
除了前述围绕会话交互展开的 4 大支柱外，DuMemory 在系统基座层还构建了 5 大底层能力，负责环境感知、知识自举、历史挖掘与运维托管：

### 5.1 宿主终端 UI / 状态栏扩展（TUI Statusline）

- **核心职责**：通过 stdout 供宿主终端 TUI 实时捕获，在界面底部渲染指示器（如 `DuMemory · <bankId>`）。纯本地读取配置，零网络调用，绝不引入 UI 卡顿。
...
- **核心职责**：代码库冷启动时，在后台以 Detached 方式拉起无头 Agent 探针，在严格只读沙箱（彻底禁用 Bash/Write/Edit）中漫游代码库，自动调用 `dumemory_ingest_document` 生成 4 篇基线架构文档（组件地图、核心概念、代码规范、技术栈特性），并自动过滤 live 提示词文件（`CLAUDE.md`, `AGENTS.md`, `.cursorrules` 等）。在 OpenCode 中甚至直接向宿主配置注入了专用的只读子 Agent 声明（`dumemory-survey`）。
...
- **支持范围与“自动级联借用”机制**：
  - **直接执行引擎（Survey Runner，共 4 个）**：**Claude Code** (`claude -p`), **Codex CLI** (`codex exec --sandbox read-only`), **Antigravity CLI** (`agy -p --mode=plan`), **OpenCode** (`opencode run --agent dumemory-survey`)；
  - **其余 12 个 Agent（Cursor, Copilot, Devin, Grok, Qwen, Cline, Kilo, Pi, Prime, Dcode 等）的级联借用**：
    这些宿主因本质是图形 IDE / 交互终端，缺乏外部脚本可调用的只读单次无头 CLI 接口（或像 Dcode 在只读模式下阻断写工具）。当在这些宿主中遇到冷库时，插件会**自动探测系统 `PATH` 中是否安装了前述 4 个 CLI 之一**（按 Claude ➔ Codex ➔ Antigravity ➔ OpenCode 优先级探测），借用其后台进程静默完成调研。启动时会通过环境变量注入当前仓库路径（`DUMEMORY_MCP_PROJECT_CWD`），**生成的架构文档精准存入当前宿主绑定的 Bank 中**；若 4 个工具均未安装，则**优雅跳过（Fail-open）**，依靠 Git Commit Seed 托底。
...
- **核心职责**：更新目标为**客户端本地胶水层代码**（`~/.dumemory/coding-agents/dist`）。每 24 小时检查一次 npm 注册表，发现新版本时后台拉取并原地热替换暂存区文件，使全机所有 Agent 下次启动自动生效。
- **触发时机与频次**：**每 24 小时在会话启动时后台静默检查 1 次**。
- **作用范围**：**全机全局共享（Host-wide / All Repos）**。所有 Agent 共享同一套暂存包。
- **为何云端模式下依然必要**：后端数据虽在云端，但各大 Agent 本地的 Hook、MCP 桥接器与日志解析器依然运行在本地客户端；各 Agent 宿主（如 Cursor、Claude）的小版本迭代频繁，本地胶水层代码必须持续跟进以修复适配 Bug。
...
| **5. 客户端静默自更新**   | **每 24h 会话启动时检查 1 次**                        | **全机全局共享** (统一暂存运行时)        | `auto-update.ts`            | **全部 16 个 Agent**                                               | —                                                            | —                                                                                        |
...
执行安装（`npx @baiducloud/dumemory-coding-agents install <agent>`）或日常运行时，插件会在宿主用户操作系统（`~` 目录）中管理、读写或扫描以下具体路径：

### 7.1 DuMemory 自身公共基础目录

- **`~/.dumemory/coding-agent.json`**：**全局核心配置文件**。存储 `serverMode`（仅 `cloud`）、API Token、服务端 URL 以及仓库级别路由覆盖等。
  > [!NOTE]
...
| :-------------------------------------- | :------------------------- | :-------------------------- | :---------------------------------------------------------------------------------- |
| **`--api-url <url>`**                   | `apiUrl`                   | `DUMEMORY_API_URL`          | 服务端 HTTP 地址。默认 Baidu AI Cloud 官方网关；指向私有化部署时填写                |
| **`--api-token <token>`**               | `apiToken`                 | `DUMEMORY_API_TOKEN`        | 访问鉴权 Bearer Token，必填                                                         |
...
#### 7.1.2 配置文件运行时高级参数全景表（`~/.dumemory/coding-agent.json`）

| 功能分类          | 配置字段 (JSON Key)               | 默认值                         | 对应环境变量 (Fallback)             | 作用与机制说明                                                                     |
| :---------------- | :-------------------------------- | :----------------------------- | :---------------------------------- | :--------------------------------------------------------------------------------- |
| **工作区与路由**  | `bankIdTemplate`                  | `"coding-agent::{gitProject}"` | `DUMEMORY_BANK_ID_TEMPLATE`         | 动态 Bank 命名模板，确保多项目/多仓库记忆物理隔离                                  |
|                   | `bankId`                          | _null (动态)_                  | `DUMEMORY_BANK_ID`                  | 强制全局固定使用单一 Bank（设置后关闭动态路由）                                    |
|                   | `mapPathToBank`                   | `{}`                           | — (仅文件)                          | 路径显式映射表，如`{"/repo/path": "custom-bank"}`，优先级最高                      |
|                   | `optInOnly` / `optInPaths`        | `false` / `[]`                 | `DUMEMORY_OPT_IN_ONLY`              | 白名单机制。开启后仅对指定目录开启记忆，其余项目静默                               |
|                   | `resolveWorktrees`                | `true`                         | `DUMEMORY_RESOLVE_WORKTREES`        | Git Worktree 是否自动与主仓库共享同一个 Memory Bank                                |
| **架构知识页面**  | `pageTriggerType`                 | `"auto-refresh"`               | `DUMEMORY_PAGE_TRIGGER_TYPE`        | 架构页合成触发模式：`auto-refresh`（自动）、`cron`（定时）、`manual`（纯手动）     |
|                   | `pageTriggerCron`                 | _null_                         | `DUMEMORY_PAGE_TRIGGER_CRON`        | 当`pageTriggerType` 为 `cron` 时的 UTC 调度表达式（如 `"0 3 * * *"`）              |
|                   | `pageRefreshEveryTurns`           | `10`                           | `DUMEMORY_PAGE_REFRESH_EVERY_TURNS` | 长会话中每隔多少轮发问刷新一次知识页面名录（防疲劳与遗忘）                         |
| **Git历史与调研** | `gitIngest`                       | `"message"`                    | `DUMEMORY_GIT_INGEST`               | Git 挖掘深度：`message`（提交信息）、`full`（单 Commit 完整 Diff）、`none`（关闭） |
|                   | `autoSeed` / `seedLimit`          | `true` / `300`                 | `DUMEMORY_AUTO_SEED`                | 冷库首次启动时，抓取最近 N 条 Git Commit 建立初始知识底座                          |
|                   | `codebaseSurvey`                  | `true`                         | `DUMEMORY_CODEBASE_SURVEY`          | 冷库首次启动时，是否拉起无头 Agent 探针漫游代码库生成 4 篇架构文档                 |
|                   | `surveyModel` / `surveyBudgetUsd` | `"haiku"` / `2`                | `DUMEMORY_SURVEY_MODEL`             | 无头调研使用的模型与单次调研花费上限（美元）                                       |
|                   | `surveyRefreshCommits`            | `20`                           | `DUMEMORY_SURVEY_REFRESH_COMMITS`   | 累计产生多少个新 Commit 后自动增量重新触发代码库调研                               |
| **推理与拦截**    | `autoReflect`                     | `true`                         | `DUMEMORY_AUTO_REFLECT`             | 用户首轮提问时，是否自动推理并注入`<dumemory_memory>`（关闭则转为纯工具调用）      |
|                   | `reflectToolTimeoutMs`            | `330000` (ms)                  | `DUMEMORY_REFLECT_TOOL_TIMEOUT_MS`  | 模型主动调用`dumemory_reflect` 工具时的硬超时上限（毫秒）                          |
|                   | `reflectBudget`                   | `"high"`                       | `DUMEMORY_REFLECT_BUDGET`           | 模型主动调用`dumemory_reflect` 工具时的推理预算：`low` / `mid` / `high`            |
|                   | `retainSessions`                  | `true`                         | `DUMEMORY_RETAIN_SESSIONS`          | 会话结束时是否将对话记录写回记忆库（设为`false` 则为只读模式）                     |
...
| **运维与安全**    | `autoUpdate`                      | `true`                         | `DUMEMORY_AUTO_UPDATE`              | 是否每 24h 检查一次 npm 自动静默热替换运行时；离线环境设为`false`                  |
|                   | `disabled`                        | `false`                        | `DUMEMORY_DISABLED`                 | 插件全局硬关停（完全静默，用于对比无记忆基线）                                     |
|                   | `logLevel`                        | `"info"`                       | `DUMEMORY_LOG_LEVEL`                | 本地日志输出级别：`"debug"` / `"info"` / `"warn"` / `"error"`                      |

> [!NOTE]
> **配置生效优先级（Layering Order，后者覆盖前者）**：
>
> 1. **内置默认值 (Built-in Defaults)**
> 2. **环境变量 (`DUMEMORY_*`)**（容器/CI 场景推荐）
> 3. **配置文件顶层 (`~/.dumemory/coding-agent.json` Top-level)**（命令行安装写入此处）
> 4. **配置文件专属分部 (`harnesses.<name>`)**（当前 Agent 专属定制，优先级最高）

- **`~/.dumemory/coding-agents/`**：**运行时 Staging 暂存目录**。安装时将当前打包的 `dist/`、`skill/`、`package.json` 复制到此，自动热更新机制（`autoUpdate`）在后台检测到 npm 新版本后也会在此完成静默更新，确保各 Agent 引用的路径永久稳定。
- **`~/.dumemory/logs/`**：插件运行与诊断日志存储路径。
- **`~/.agents/skills/dumemory-coding-agent/`**：**AgentSkills 跨工具共享技能标准根目录**。供遵循该规范的 Agent（Codex、DeepSeek Harness）自动发现 DuMemory 记忆技能。

> [!TIP]
> **技能路径与卸载隔离设计**：
> 宿主如果支持私有技能目录（如 Claude Code 的 `~/.claude/skills/`、Antigravity 的 `~/.gemini/config/skills/` 等），安装器会**严格将技能写入其各自的私有目录**。
> 这是因为 `uninstall <agent>` 是按固定名称 `dumemory-coding-agent` 进行清理的；如果将所有 Agent 都安装到 `~/.agents/skills/` 共享路径，卸载某一 Agent 时就会**连带误删**其他 Agent 的技能副本。同时，每次会话启动时 `src/core/skill-sync.ts` 还会自动比对各私有/共享目录内的 `SKILL.md`，实现静默热更新。
...
| :------------------------- | :---------------------------------------------------------- | :------------------------------------------------ | :----------------------------------------------------- | :------------------------------------------------------------------------- |
| **Claude Code**            | `~/.claude/settings.json`                                   | 经`claude mcp add` 注册至用户级全局 MCP 配置      | `~/.claude/skills/dumemory-coding-agent/`              | `~/.claude/projects/<slugified-cwd>/<sessionId>.jsonl`                     |
| **Cursor CLI**             | `~/.cursor/hooks.json`                                      | `~/.cursor/mcp.json`                              | `~/.cursor/skills/dumemory-coding-agent/`              | 钩子事件 payload 或当前工作区状态转录                                      |
| **GitHub Copilot CLI**     | `~/.copilot/hooks/dumemory-coding-agents.json`              | `~/.copilot/mcp-config.json`                      | `~/.copilot/skills/dumemory-coding-agent/`             | 钩子事件上下文传输之转录流                                                 |
| **OpenAI Codex CLI**       | `~/.codex/hooks.json` 与 `~/.codex/config.toml`             | `~/.codex/config.toml` (`[mcp_servers.dumemory]`) | `~/.agents/skills/dumemory-coding-agent/` (共享)       | `~/.codex/sessions/` 或通过 Hook payload 读取                              |
| **Google Antigravity CLI** | `~/.gemini/config/hooks.json` (命名空间：`coding-agents`)   | `~/.gemini/config/mcp_config.json`                | `~/.gemini/config/skills/dumemory-coding-agent/`       | `~/.gemini/antigravity/brain/<id>/.system_generated/logs/transcript.jsonl` |
...
| **Grok Build**             | `~/.grok/config.toml` (`[[hooks.*]]` 块)                    | `~/.grok/config.toml` (`[mcp_servers.dumemory]`)  | `~/.grok/skills/dumemory-coding-agent/`                | `~/.grok/sessions/<url-encoded-cwd>/<sessionId>`                           |
| **Qwen Code**              | `~/.qwen/settings.json`                                     | 经`qwen mcp add` 注册至用户级配置                 | `~/.qwen/skills/dumemory-coding-agent/`                | `~/.qwen/projects/<slugified-cwd>/chats/<sessionId>.jsonl`                 |
...
| **Cline CLI**              | 经`cline plugin install` 安装于内部插件目录                 | `~/.cline/data/settings/cline_mcp_settings.json`  | `~/.cline/data/settings/skills/dumemory-coding-agent/` | 进程内拦截（通过`beforeModel` 钩子）                                       |
| **DeepSeek Harness (DSH)** | `$DSH_HOME/cordis.patch.yml` (或 `~/.dsh/cordis.patch.yml`) | 由 Cordis 内部机制解析                            | `~/.agents/skills/dumemory-coding-agent/` (共享)       | 遵循 DSH 会话记录或 Hook 上下文处理                                        |
| **Pi**                     | `~/.pi/agent/settings.json` (`extensions` 字段)             | 原生工具注入                                      | `~/.pi/agent/skills/dumemory-coding-agent/`            | `~/.pi/agent/sessions/**/*.jsonl`                                          |
| **Prime Agent**            | `~/.prime/agent/settings.json` (`extensions` 字段)          | 原生工具注入                                      | `~/.prime/agent/skills/dumemory-coding-agent/`         | `~/.prime/agent/sessions/`                                                 |
...
- **单元与集成测试（Vitest）**：
  - 配置文件：`hindsight-integrations/coding-agents-dumemory/vitest.config.ts`
  - 测试用例目录：`hindsight-integrations/coding-agents-dumemory/src/__tests__/` 及与源码同级的 `*.test.ts` 文件（如 `installer.test.ts`、`mcp-server.test.ts` 等），覆盖配置解析、会话缓存与工具行为。
- **端到端多 Agent 沙箱测试（E2E Docker）**：
  - 目录：`hindsight-integrations/coding-agents-dumemory/e2e/` 与 `src/e2e/`
  - 驱动脚本：`run-harness.sh`，结合包含各 Agent 隔离环境的 Dockerfile（`Dockerfile.claude-code`、`Dockerfile.dcode` 等）进行真实的生命周期端到端集成验证。
- **说明**：根目录的 `dumemory-integration-tests/` 专门用于 DuMemory 后端 Python FastAPI 服务的集成测试，不测试 coding-agents 插件。
