/**
 * Static route handler for listing chunks of a document.
 *
 * Architectural Decision (Issue #4586, #4587):
 * Document IDs frequently contain slashes (e.g. file paths like `folder/doc.pdf`).
 * Previously, this endpoint was defined under dynamic path `/api/documents/[documentId]/chunks`.
 ...
 *
 * By routing via a static path `/api/documents/chunks` and reading `document_id` from URL query
 * parameters (`?bank_id=...&document_id=...`), this endpoint is immune to path normalization.
 *
 * DO NOT refactor this back to `/api/documents/[documentId]/chunks`.
 */
