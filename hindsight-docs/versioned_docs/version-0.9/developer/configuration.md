...
| `HINDSIGHT_API_CONSOLIDATION_WALL_TIMEOUT` | Wall-clock ceiling in seconds for one consolidation task in the worker. A wedged consolidation is cancelled and marked `failed` instead of holding its worker slot indefinitely, so it can be retried. Set above the longest healthy consolidation round; `0` disables. | `7200` |
